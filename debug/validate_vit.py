...
with torch.no_grad():
    ...
    for batch_idx in tqdm(range(len(dataloader))):
        ...
        dispatch_for_generation(model.thinker.visual)
        ret = flatten_obj(model.thinker.visual(**inputs))
    ...
    for batch_idx in tqdm(range(len(dataloader))):
        # inputs = {k: v.to(model_device) for k, v in inputs.items()}
        inputs = activations.fetch(batch_idx)
        dispatch_for_generation(ref_model.thinker.visual)
        ref_ret = flatten_obj(ref_model.thinker.visual(**inputs))
