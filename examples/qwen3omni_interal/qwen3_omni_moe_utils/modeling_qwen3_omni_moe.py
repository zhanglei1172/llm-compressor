...
class Qwen3_VLVisionSdpaAttention(nn.Module):
    ...
    def forward(
        ...
        for i in range(len(cu_seqlens) - 1):
            ...
            attn_output_window = F.scaled_dot_product_attention(
                q_window, k_window, v_window, dropout_p=0.0
            )
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        for expert_idx in range(self.num_experts):
            ...
            current_state = expert_layer(
                hidden_states[None, top_x].reshape(-1, hidden_dim)
...
class Qwen3_VL_MOEModel(Qwen3_VL_MOEPreTrainedModel):
    def __init__(self, config: Qwen3_VL_MoeConfig):
        ...
        self.layers = nn.ModuleList(
            [
                Qwen3_VL_MoeDecoderLayer(config, layer_idx)
    ...
    def _deepstack_process(self, hidden_states, visual_pos_masks, visual_embeds_this):
        ...
        visual_embeds_this = visual_embeds_this.to(
    ...
    def forward(
        ...
        if position_ids is None:
            ...
            position_ids = (
                position_ids[None, ...].expand(3, position_ids.shape[0], -1).float()
            )
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if deepstack_visual_embeds_multiscale is not None and layer_idx in range(
                ...
                self.deepstack_multiscale_layer_start
                + len(deepstack_visual_embeds_multiscale[0]),
            ):
                deepstack_visual_embeds_multiscale_this = (
                    deepstack_visual_embeds_multiscale[
                        0, layer_idx - self.deepstack_multiscale_layer_start
                    ]
                )
                if visual_pos_masks is None:
                    hidden_states = (
                        hidden_states + deepstack_visual_embeds_multiscale_this
                    )
                else:
...
class Qwen3_VL_VisionTransformerPretrainedModel(Qwen3_VLPreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        self.deepstack_visual_multiscale_indexes = (
            config.deepstack_visual_multiscale_indexes
        )
        self.gradient_checkpointing = False
    ...
    def fast_pos_embed_interpolate(self, grid_ts, grid_hs, grid_ws):
        ...
        patch_pos_embeds = patch_pos_embeds.split(
            [t * h * w for t, h, w in zip(grid_ts, grid_hs, grid_ws)]
        )
    ...
    def forward(
        ...
        deepstack_visual_multiscale_indexes = (
            self.config.deepstack_visual_multiscale_indexes
        )
        ...
        for layer_num, blk in enumerate(self.blocks):
            ...
            if (
                deepstack_visual_multiscale_indexes is not None
                and layer_num in deepstack_visual_multiscale_indexes
            ):
...
class Qwen3OmniMoePreTrainedModelForConditionalGeneration(Qwen3OmniMoePreTrainedModel):
    ...
    def get_llm_pos_ids_for_vision(
        ...
        h_index = (
            torch.arange(llm_grid_h, device=grid_hs.device)
            .view(1, -1, 1)
            .expand(len(t_index), -1, llm_grid_w)
            .flatten()
        )
        w_index = (
            torch.arange(llm_grid_w, device=grid_ws.device)
            .view(1, 1, -1)
        ...
        if input_ids is not None and (
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                for _ in range(multimodal_nums):
                    st_idx = (
                        llm_pos_ids_list[-1].long().float().max() + 1
                    ...
                    if min_ed == ed_audio_start:
                        ...
                        t_index = (
                            (torch.arange(grid_t, device=device) + shift)
                ...
                position_ids[..., i, attention_mask[i] == 1] = llm_positions.float().to(
                    position_ids.device
                )
                mrope_position_deltas.append(
                    llm_positions.long().float().max() + 1 - len(input_ids)
                )
    ...
    def _get_llm_pos_ids_for_vision(
        ...
        w_index = (
            torch.arange(llm_grid_w, device=llm_grid_h.device)
            .view(1, 1, -1)
            .expand(len(t_index), llm_grid_h, -1)
            .flatten()
        )
...
class Qwen3OmniMoeAudioEncoder(Qwen3OmniMoePreTrainedModel):
    ...
    def __init__(self, config: Qwen3OmniMoeAudioEncoderConfig):
        ...
        self.conv_out = nn.Linear(
            480 * ((((config.num_mel_bins + 1) // 2 + 1) // 2 + 1) // 2),
            config.d_model,
            bias=False,
        )
...
class Qwen3OmniMoeThinkerForConditionalGeneration(
    ...
    _no_split_modules = [
        ...
        "Qwen3_VL_VisionTransformerPretrainedModel",
    ]
    ...
    def __init__(self, config: Qwen3OmniMoeThinkerConfig):
        ...
        self.visual = Qwen3_VL_VisionTransformerPretrainedModel._from_config(
    ...
    def forward(
            ...
            if past_key_values is None:
                ...
                for i, kv in enumerate(past_key_values):
                    if kv is None:
                        ...
                        print(
                            f"  past_key_values[{i}]: shape={tuple(kv[0].shape)}, dtype={kv[0].dtype}"
                        )
        ...
        if use_deepstack:
            ...
            if pixel_values is not None or pixel_values_videos is not None:
                input_embeds_multiscale = (
                    ...
                    .unsqueeze(-2)
                    .repeat(
                        1, 1, len(self.visual.deepstack_visual_multiscale_indexes), 1
                    )
                    .flatten(-2)
                )
        ...
        if pixel_values is not None:
            ...
            if use_deepstack:
                image_embeds, embeddings_multiscale = image_embeds.split(
                    [visual_dim, visual_dim * multiscale_len], dim=-1
                )
                input_embeds_multiscale = input_embeds_multiscale.masked_scatter(
        ...
        if pixel_values_videos is not None:
            ...
            if use_deepstack:
                video_embeds, embeddings_multiscale = video_embeds.split(
                    [visual_dim, visual_dim * multiscale_len], dim=-1
                )
                input_embeds_multiscale = input_embeds_multiscale.masked_scatter(
                    (input_ids == self.config.video_token_id).unsqueeze(-1),
                    embeddings_multiscale,
                )
        ...
        if use_deepstack and input_embeds_multiscale is not None:
            input_embeds_multiscale = (
                input_embeds_multiscale.view(
                    -1, inputs_embeds.shape[1], multiscale_len, visual_dim
                )
                .permute(0, 2, 1, 3)
                .contiguous()
            )
...
class Qwen3OmniMoeTalkerModel(Qwen3OmniMoePreTrainedModel):
    ...
    def __init__(self, config: Qwen3OmniMoeTalkerConfig):
        ...
        self.layers = nn.ModuleList(
            [
                Qwen3OmniMoeDecoderLayer(config, layer_idx)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self._attn_implementation = config._attn_implementation
        ...
        self.has_sliding_layers = (
            False  # "sliding_attention" in self.config.layer_types
        )
...
class DiTDecoderLayer(nn.Module):
    def __init__(
        self, config: Qwen3OmniMoeDiTConfig, look_ahead_block=0, look_backward_block=0
    ):
        super().__init__()
...
class Qwen3OmniMoeToken2WavDiTModel(Qwen3OmniMoePreTrainedModel):
    ...
    def __init__(self, config: Qwen3OmniMoeDiTConfig):
        ...
        self.norm_out = Qwen3_OmniAdaLayerNormZero_Final(
            config.hidden_size
        )  # final modulation
...
class Qwen3OmniMoeToken2WavModel(Qwen3OmniMoePreTrainedModel):
    ...
    base_model_prefix = "model"
    _no_split_modules = [
        "Qwen3OmniMoeToken2WavDiTModel",
        "Qwen3OmniMoeToken2WavBigVGANModel",
    ]
