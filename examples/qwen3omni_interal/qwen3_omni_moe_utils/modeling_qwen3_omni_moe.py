...
from .configuration_qwen3_omni_moe import (
    Qwen3_VL_Config,
    Qwen3_VL_MoeConfig,
    Qwen3_VL_VisionConfig,
    Qwen3OmniMoeAudioEncoderConfig,
    Qwen3OmniMoeBigVGANConfig,
    Qwen3OmniMoeConfig,
    Qwen3OmniMoeDiTConfig,
    Qwen3OmniMoeTalkerConfig,
    Qwen3OmniMoeTextConfig,
    Qwen3OmniMoeThinkerConfig,
    Qwen3OmniMoeToken2WavConfig,
)
...
# Modified from ...models.mistral.modeling_mistral.MistralMLP with Mistral->Qwen2Moe
...
class Qwen2MoeAttention(nn.Module):
    ...
    def __init__(self, config: Qwen3_VL_MoeConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        ...
        self.q_proj = nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=getattr(config, "qkv_bias", False))
        self.k_proj = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=getattr(config, "qkv_bias", False))
        self.v_proj = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=getattr(config, "qkv_bias", False))
...
class Qwen2MoeFlashAttention2(Qwen2MoeAttention):
    ...
    # Copied from ...models.llama.modeling_llama.LlamaFlashAttention2.__init__
...
QWEN3_VL_VISION_ATTENTION_CLASSES = {
...
class Qwen2MoeSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        for expert_idx in range(self.num_experts):
            ...
            current_state = expert_layer(hidden_states[None, top_x].reshape(-1, hidden_dim)).to(hidden_states.device)
...
class Qwen3_VL_MoeDecoderLayer(nn.Module):
    def __init__(self, config: Qwen3_VL_MoeConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class Qwen2_5_VLRotaryEmbedding(nn.Module):
    def __init__(self, config: Qwen3_VL_MoeConfig, device=None):
        super().__init__()
...
class Qwen3_VL_MOEPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["Qwen3_VL_MoeDecoderLayer", "Qwen3_VL_VisionBlock"]
...
class Qwen3_VL_MOEModel(Qwen3_VL_MOEPreTrainedModel):
    def __init__(self, config: Qwen3_VL_MoeConfig):
        ...
        self.layers = nn.ModuleList(
            [Qwen3_VL_MoeDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    def _deepstack_process(self, hidden_states, visual_pos_masks, visual_embeds_this):
        ...
        local_this = hidden_states[visual_pos_masks, :].clone() + visual_embeds_this
    ...
    def forward(
        ...
        if position_ids is None:
            position_ids = cache_position.view(1, 1, -1).expand(3, inputs_embeds.shape[0], -1).float()
        elif position_ids.dim() == 2:
            position_ids = position_ids[None, ...].expand(3, position_ids.shape[0], -1).float()
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if deepstack_visual_embeds_multiscale is not None and layer_idx in range(
                ...
                self.deepstack_multiscale_layer_start + len(deepstack_visual_embeds_multiscale[0]),
            ):
                deepstack_visual_embeds_multiscale_this = deepstack_visual_embeds_multiscale[0,
                    layer_idx - self.deepstack_multiscale_layer_start
                ]
                if visual_pos_masks is None:
                    hidden_states = hidden_states + deepstack_visual_embeds_multiscale_this
                else:
...
class Qwen3_VL_VisionBlock(nn.Module):
    def __init__(self, config, attn_implementation: str = "flash_attention_2") -> None:
        ...
        self.attn = QWEN3_VL_VISION_ATTENTION_CLASSES[attn_implementation](
...
class Qwen3_VLPatchMerger(nn.Module):
    def __init__(self, dim: int, context_dim: int, spatial_merge_size: int = 2, use_rmsnorm=False, use_postshuffle_norm=False) -> None:
        super().__init__()
...
class Qwen3_VL_VisionTransformerPretrainedModel(Qwen3_VLPreTrainedModel):
    config_class = Qwen3_VL_VisionConfig
    _no_split_modules = ["Qwen3_VL_VisionBlock"]
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        if config.apply_vit_abs_pos_embed:
            ...
            self.pos_embed = nn.Parameter(torch.empty([1, (config.img_size // config.spatial_patch_size) ** 2, config.hidden_size]))
        ...
        if config.deepstack_visual_multiscale_indexes is not None:
            self.merger_list = nn.ModuleList([
                ...
                ) for _ in range(len(config.deepstack_visual_multiscale_indexes))
            ])

        self.deepstack_visual_multiscale_indexes = config.deepstack_visual_multiscale_indexes
        self.gradient_checkpointing = False
    ...
    def interpolate_pos_encoding(self, t: int, height: int, width: int, pos_embed = None) -> torch.Tensor:
        m_size = self.config.spatial_merge_size
        ...
        patch_pos_embed = patch_pos_embed.view(height // m_size, m_size , width // m_size, m_size, -1).permute(0, 2, 1, 3, 4).flatten(0, 3)
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        hidden_states_list = ()
        deepstack_visual_multiscale_indexes = self.config.deepstack_visual_multiscale_indexes
        ...
        for layer_num, blk in enumerate(self.blocks):
            ...
            if deepstack_visual_multiscale_indexes is not None and layer_num in deepstack_visual_multiscale_indexes:
                hidden_states_list = hidden_states_list + (hidden_states,)

        hidden_states = self.merger(hidden_states)

        # # processing deepstack
        # if deepstack_visual_multiscale_indexes is not None:
...
class Qwen3OmniMoePreTrainedModelForConditionalGeneration(Qwen3OmniMoePreTrainedModel):
    ...
    def get_llm_pos_ids_for_vision(
        ...
        h_index = torch.arange(llm_grid_h, device=grid_hs.device).view(1, -1, 1).expand(len(t_index), -1, llm_grid_w).flatten()
        w_index = torch.arange(llm_grid_w, device=grid_ws.device).view(1, 1, -1).expand(len(t_index), llm_grid_h, -1).flatten()
        ...
        if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                for _ in range(multimodal_nums):
                    st_idx = llm_pos_ids_list[-1].long().float().max() + 1 if len(llm_pos_ids_list) > 0 else 0
                    ...
                    if min_ed == ed_audio_start:
                        ...
                        t_index = ((torch.arange(grid_t,device=device)) * 1 * position_id_per_seconds).float()
                        ...
                        t_index = (
                            (torch.arange(grid_t,device=device) + shift) * second_per_grids[video_idx].cpu().float() * position_id_per_seconds
                        ).float()
                        llm_pos_ids = self._get_llm_pos_ids_for_vision(
                ...
                position_ids[..., i, attention_mask[i] == 1] = llm_positions.float().to(position_ids.device)
                mrope_position_deltas.append(llm_positions.long().float().max() + 1 - len(input_ids))
    ...
    def _get_llm_pos_ids_for_vision(
        ...
        llm_grid_h = grid_hs[vision_idx] // spatial_merge_size
        llm_grid_w = grid_ws[vision_idx] // spatial_merge_size
        h_index = (torch.arange(llm_grid_h, device=llm_grid_h.device).view(1, -1, 1).expand(
            len(t_index), -1, llm_grid_w).flatten())
        w_index = (torch.arange(llm_grid_w, device=llm_grid_h.device).view(1, 1, -1).expand(
            len(t_index), llm_grid_h, -1).flatten())
        
        t_index_tensor = (t_index).to(llm_grid_h.device).view(
            -1, 1).expand(-1, llm_grid_h * llm_grid_w).float().flatten()
...
class Qwen3OmniMoeAudioEncoder(Qwen3OmniMoePreTrainedModel):
    ...
    def my_wrap_0(self, input_features, chunk_num, feature_lens, padding_value=0, padding_side="right"):
        ...
        padded_feature, padded_mask, padded_mask_after_cnn = self.padded_and_mask_function(
            chunk_list, chunk_lengths, padding_value=padding_value, padding_side=padding_side
        )
    ...
    def forward(
        ...
        padded_feature, padded_mask, padded_mask_after_cnn = self.my_wrap_0(
            input_features, chunk_num, feature_lens, padding_value=0, padding_side="right"
        )
        padded_feature = (padded_feature * padded_mask).unsqueeze(1)
        ...
        if hasattr(self.positional_embedding, "positional_embedding"):
            padded_embed = padded_embed + self.positional_embedding.positional_embedding[
                : padded_embed.shape[1], :
            ].unsqueeze(0).to(padded_embed.dtype)
        else:
            padded_embed = padded_embed + self.positional_embedding.weight[
                : padded_embed.shape[1], :
            ].unsqueeze(0).to(padded_embed.dtype)
        hidden_states = padded_embed[padded_mask_after_cnn]
        cu_seqlens = self.my_wrap_1(aftercnn_lens, padded_mask_after_cnn)
...
class Qwen3OmniRotaryEmbedding(nn.Module):
    ...
    @dynamic_rope_update  # power user: used with advanced RoPE types (e.g. dynamic rope)
    def forward(self, x, position_ids):
        # In contrast to other models, Qwen3OmniMoe has different position ids for the grids
        # So we expand the inv_freq to shape (3, ...)
        inv_freq_expanded = self.inv_freq[None, None, :, None].float().expand(3, position_ids.shape[1], -1, 1)
        position_ids_expanded = position_ids[:, :, None, :].float()  # shape (3, bs, 1, positions)
...
class Qwen3OmniMoeAttention(nn.Module):
    ...
    def __init__(self, config: Qwen3OmniMoeConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        if layer_idx is None:
...
class Qwen3OmniMoeDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Qwen3OmniMoeTextConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        self.self_attn = Qwen3OmniMoeAttention(config, layer_idx)

        self.mlp = Qwen2MLP(config)
        self.input_layernorm = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.attention_type = "full_attention" #config.layer_types[layer_idx]
...
class Qwen3OmniMoeThinkerForConditionalGeneration(Qwen3OmniMoePreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    _no_split_modules = ["Qwen3OmniMoeAudioEncoder", "Qwen3_VL_VisionTransformerPretrainedModel"]
    ...
    def __init__(self, config: Qwen3OmniMoeThinkerConfig):
        ...
        self.visual = Qwen3_VL_VisionTransformerPretrainedModel._from_config(config.vision_config)

        self.vocab_size = config.text_config.vocab_size
        self.model = Qwen3_VL_MOEModel(config.text_config)
    ...
    def get_video_features(
        ...
        sizes = video_grid_thw.prod(-1) // merge_size // merge_size
    ...
    def forward(
        ...
        if 1==2:
            ...
            for name, tensor in tensor_params.items():
                if tensor is None:
                    print(f"{name}: None")
                else:
                    print(f"{name}: shape={tuple(tensor.shape)}, dtype={tensor.dtype}")

            # List parameter (past_key_values)
            if past_key_values is None:
                print("past_key_values: None")
            else:
                print(f"past_key_values: list of length {len(past_key_values)}")
                for i, kv in enumerate(past_key_values):
                    if kv is None:
                        print(f"  past_key_values[{i}]: None")
                    else:
                        print(f"  past_key_values[{i}]: shape={tuple(kv[0].shape)}, dtype={kv[0].dtype}")
        ...
        use_deepstack = self.visual.deepstack_visual_multiscale_indexes is not None
        input_embeds_multiscale = None
        if use_deepstack:
            multiscale_len = len(self.visual.deepstack_visual_multiscale_indexes)
            visual_dim = inputs_embeds.shape[-1]
            if pixel_values is not None or pixel_values_videos is not None:
                input_embeds_multiscale = torch.zeros_like(inputs_embeds).unsqueeze(-2).repeat(1, 1, len(self.visual.deepstack_visual_multiscale_indexes), 1).flatten(-2)
        ...
        if pixel_values is not None:
            ...
            if use_deepstack:
                image_embeds, embeddings_multiscale = image_embeds.split([visual_dim, visual_dim * multiscale_len], dim=-1)
                input_embeds_multiscale = input_embeds_multiscale.masked_scatter((input_ids == self.config.image_token_id).unsqueeze(-1), embeddings_multiscale)
        ...
        if pixel_values_videos is not None:
            ...
            if use_deepstack:
                video_embeds, embeddings_multiscale = video_embeds.split([visual_dim, visual_dim * multiscale_len], dim=-1)
                input_embeds_multiscale = input_embeds_multiscale.masked_scatter((input_ids == self.config.video_token_id).unsqueeze(-1), embeddings_multiscale)
            inputs_embeds = inputs_embeds.masked_scatter(video_mask, video_embeds)
        
        if use_deepstack and input_embeds_multiscale is not None:
            input_embeds_multiscale = input_embeds_multiscale.view(-1, inputs_embeds.shape[1], multiscale_len, visual_dim).permute(0,2,1,3).contiguous()
        ...
        outputs = self.model(
            ...
            cache_position=cache_position,
            # deepstack args
            deepstack_visual_embeds_multiscale=input_embeds_multiscale,
...
class Qwen3OmniMoeTalkerCausalLMOutputWithPast(ModelOutput):
    r"""
    loss (`torch.FloatTensor` of shape `(1,)`, *optional*, returned when `labels` is provided):
        Language modeling loss (for next-token prediction).
...
class Qwen3OmniMoeTalkerModel(Qwen3OmniMoePreTrainedModel):
    ...
    def __init__(self, config: Qwen3OmniMoeTalkerConfig):
        ...
        self.norm = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = Qwen3OmniRotaryEmbedding(config=config)
        self.has_sliding_layers = False #"sliding_attention" in self.config.layer_types
...
class Qwen3OmniMoeTalkerForConditionalGeneration(Qwen3OmniMoePreTrainedModelForConditionalGeneration, GenerationMixin):
    config_class = Qwen3OmniMoeTalkerConfig
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[tuple, Qwen3OmniMoeTalkerCausalLMOutputWithPast]:
        r"""
        ...
        >>> from urllib.request import urlopen
        >>> import librosa
        >>> from transformers import AutoProcessor, Qwen3OmniMoeTalkerForConditionalGeneration

        >>> model = Qwen3OmniMoeTalkerForConditionalGeneration.from_pretrained("Qwen/Qwen2-Audio-7B")
        ...
        return Qwen3OmniMoeTalkerCausalLMOutputWithPast(
            loss=loss,
            logits=logits,
            past_key_values=outputs.past_key_values,
            hidden_states=hidden_states,
...
class Qwen3OmniMoeDiTRotaryEmbedding(nn.Module):
...
class ECAPA_TimeDelayNet(torch.nn.Module):
    ...
    def __init__(self, config: Qwen3OmniMoeDiTConfig):
        super().__init__()
...
class Qwen3_OmniAdaLayerNormZero(nn.Module):
...
class Qwen3_OmniAdaLayerNormZero_Final(nn.Module):
    def __init__(self, dim):
        super().__init__()
...
class DiTDecoderLayer(nn.Module):
    def __init__(self, config: Qwen3OmniMoeDiTConfig, look_ahead_block=0, look_backward_block=0):
        super().__init__()
        self.attn_norm = Qwen3_OmniAdaLayerNormZero(config.hidden_size)
...
class Qwen3OmniMoeToken2WavBigVGANModel(Qwen3OmniMoePreTrainedModel):
    config_class = Qwen3OmniMoeBigVGANConfig

    def __init__(self, config: Qwen3OmniMoeBigVGANConfig):
        super().__init__(config)
...
class Qwen3OmniMoeToken2WavDiTModel(Qwen3OmniMoePreTrainedModel):
    config_class = Qwen3OmniMoeDiTConfig
    ...
    def __init__(self, config: Qwen3OmniMoeDiTConfig):
        ...
        self.rotary_embed = Qwen3OmniMoeDiTRotaryEmbedding(config.head_dim)
        ...
        self.norm_out = Qwen3_OmniAdaLayerNormZero_Final(config.hidden_size)  # final modulation
...
class Qwen3OmniMoeToken2WavModel(Qwen3OmniMoePreTrainedModel):
    config_class = Qwen3OmniMoeToken2WavConfig
    base_model_prefix = "model"
    _no_split_modules = ["Qwen3OmniMoeToken2WavDiTModel", "Qwen3OmniMoeToken2WavBigVGANModel"]

    def __init__(self, config: Qwen3OmniMoeToken2WavConfig):
        super().__init__(config)
        attn_impl = config._attn_implementation
        if config._attn_implementation == "flash_attention_2":
            logger.warning_once(
                "Qwen3OmniMoeToken2WavModel must inference with fp32, but flash_attention_2 only supports fp16 and bf16, "
                "attention implementation of Qwen3OmniMoeToken2WavModel will fallback to sdpa."
            )
            attn_impl = "sdpa"
        elif config._attn_implementation == "eager":
            logger.warning_once(
                "Qwen3OmniMoeToken2WavModel does not support eager attention implementation, fall back to sdpa"
            )
            attn_impl = "sdpa"
        self.code2wav_dit_model = Qwen3OmniMoeToken2WavDiTModel._from_config(
            config.dit_config, attn_implementation=attn_impl
        )
        self.code2wav_bigvgan_model = Qwen3OmniMoeToken2WavBigVGANModel._from_config(
...
class Qwen3OmniMoeForConditionalGeneration(Qwen3OmniMoePreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.speaker_map = {}
        if self.has_talker and hasattr(config, "token2wav_config"):
            self.enable_talker()
        else:
    ...
    def enable_talker(self):
        ...
        self.token2wav = Qwen3OmniMoeToken2WavModel(self.config.token2wav_config)
    ...
    def generate(
        ...
        **kwargs,
        ...
        if 'spk_id' in self.config.talker_config:
