...
if pretrain == "ostq":
    ...
    MODEL_ID = "/tmp/Qwen3-Omni-Thinking-origin-spinquant(vit,)-trans"
...
ds_al = load_dataset(
    "/dataset/workspace/zhangl98/dataset/peoples_speech/test",
            ...
            while pos < len(token_ids):
                if is_assistant_response:
                    ...
                    if token_ids[pos] == self.assistant_start_tokens[0]:
                        ...
                        pos += 1
            if not valid:
                labels[batch_idx, :] = batch["input_ids"][batch_idx]
...
def patch_vit_training(model):
    ...
    def forward(module, *args, **kwargs):
        ...
        image_embeds, image_embeds_multiscale = module.get_image_features(
            pixel_values,
            image_grid_thw=image_grid_thw,
        )
        return image_embeds, *image_embeds_multiscale
...
def fsdp_main(model, config):
    ...
    if train_args.special.get("loss_type", "origin") == "mse":
        if {"aut"} == enable_modality:
            patch_audio_training(model_to_train)
        if {"vit"} == enable_modality:
            patch_vit_training(model_to_train)
    if need_teacher:
        ...
        if train_args.special.get("loss_type", "origin") == "mse":
            ...
            if {"vit"} == enable_modality:
                patch_vit_training(teacher_model)
